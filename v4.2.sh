...
# --- Bashrc beállítások - KÜLÖN KEZELÉS a komplex tartalom miatt ---
echo "Bashrc beállítása..."

# Felhasználói .bashrc létrehozása
echo "  Felhasználó: $USER_HOME/.bashrc"
...
# /etc/skel .bashrc létrehozása
echo "  /etc/skel: /etc/skel/.bashrc"
...
cp "$USER_HOME/.local/share/fonts"/* /usr/local/share/fonts/ 2>/dev/null || true

# Fontok másolása /etc/skel-be is
mkdir -p /etc/skel/.local/share/fonts
cp "$USER_HOME/.local/share/fonts"/* /etc/skel/.local/share/fonts/ 2>/dev/null || true
...
# ===========================================================================
# === MISSION IMPOSSIBLE - ROFI SYMLINK AUTOMATIZÁLÁS (JAVÍTOTT) ===
# ===========================================================================

echo "=== Mission Impossible - Rofi Symlink Automatizálás ==="

# 1. Fő mission script telepítése rendszerszintre
echo "Fő mission script telepítése..."
cat > /usr/local/bin/rofi-mission-impossible.sh << 'EOF'
#!/bin/bash

# Mission Impossible Rofi Symlink Setup
# Runs once then self-destructs

MISSION_IMPOSSIBLE() {
    local user_home="$1"
    local source_dir="$user_home/.config/rofi/scripts"
    local target_dir="$user_home/.local/bin"
    local mission_flag="$user_home/.config/rofi/.mission_accomplished"
    
    # Check if mission already accomplished
    if [ -f "$mission_flag" ]; then
        echo "⚡ Mission already accomplished - nothing to do"
        return 0
    fi
    
    echo "🕵️‍♂️ Mission Impossible: Rofi Symlink Setup"
    echo "----------------------------------------"
    
    # Create target directory
    mkdir -p "$target_dir"
    
    # Create symlinks for all executable files recursively
    if [ -d "$source_dir" ]; then
        find "$source_dir" -type f -executable | while read -r script; do
            if [ -f "$script" ]; then
                ...
                echo "✅ Symlink created: $script_name"
            fi
        done
    else
    ...
    # Create mission accomplished flag
    mkdir -p "$(dirname "$mission_flag")"
    touch "$mission_flag"
    echo "----------------------------------------"
    echo "🎯 Mission Accomplished! Symlinks created in ~/.local/bin/"
    
    # SELF-DESTRUCT SEQUENCE
    echo "💣 Activating self-destruct sequence..."
    
    # Find and remove this script from user's home
    local self_destruct_script="$user_home/.config/rofi/mission-impossible.sh"
    if [ -f "$self_destruct_script" ]; then
        rm -f "$self_destruct_script"
        echo "🔥 Mission script destroyed: $self_destruct_script"
    fi
    
    # Remove from bashrc if exists
    if grep -q "mission-impossible.sh" "$user_home/.bashrc" 2>/dev/null; then
        sed -i '/mission-impossible.sh/d' "$user_home/.bashrc"
        echo "🔥 Activation code removed from .bashrc"
    fi
    
    echo "🚀 Mission Complete. This message will self-destruct in 3...2...1..."
}
...
if [ "$EUID" -eq 0 ]; then
    if [ -n "$SUDO_USER" ]; then
        ...
        MISSION_IMPOSSIBLE "$user_home"
    else
        echo "Usage: sudo $0 or run as user"
    fi
else
    MISSION_IMPOSSIBLE "$HOME"
fi
EOF

chmod +x /usr/local/bin/rofi-mission-impossible.sh

# 2. EGYSZERŰBB AKTIVÁLÓ SCRIPT - NINCS VÉGTELEN REKURZIÓ
echo "Aktiváló script másolása /etc/skel-be..."
mkdir -p /etc/skel/.config/rofi
cat > /etc/skel/.config/rofi/mission-impossible.sh << 'EOF'
#!/bin/bash

# Mission Impossible Activator - SIMPLE VERSION
# This script runs once and self-destructs immediately

# Check if mission already completed
MISSION_FLAG="$HOME/.config/rofi/.mission_accomplished"
if [ -f "$MISSION_FLAG" ]; then
    # Clean up and exit
    rm -f "$HOME/.config/rofi/mission-impossible.sh" 2>/dev/null
    sed -i '/mission-impossible.sh/d' "$HOME/.bashrc" 2>/dev/null
    exit 0
fi

echo "🕵️‍♂️ Mission Impossible Activated - Running main mission..."

# Execute the main mission script directly
if [ -f "/usr/local/bin/rofi-mission-impossible.sh" ]; then
    bash "/usr/local/bin/rofi-mission-impossible.sh"
else
    echo "❌ Main mission script not found at /usr/local/bin/rofi-mission-impossible.sh"
fi

# IMMEDIATE SELF-DESTRUCT - no recursion
rm -f "$HOME/.config/rofi/mission-impossible.sh" 2>/dev/null
sed -i '/mission-impossible.sh/d' "$HOME/.bashrc" 2>/dev/null

echo "🔥 Mission activator self-destructed"
EOF

chmod +x /etc/skel/.config/rofi/mission-impossible.sh

# 3. Trigger hozzáadása /etc/skel/.bashrc-hez
echo "Trigger hozzáadása /etc/skel/.bashrc-hez..."
if ! grep -q "mission-impossible.sh" /etc/skel/.bashrc; then
    cat >> /etc/skel/.bashrc << 'EOF'

# Mission Impossible - One-time setup (will self-destruct)
if [ -f "$HOME/.config/rofi/mission-impossible.sh" ]; then
    bash "$HOME/.config/rofi/mission-impossible.sh"
fi
EOF
fi

# 4. Aktiváló script másolása a JELENLEGI FELHASZNÁLÓHOZ
echo "Aktiváló script másolása a jelenlegi felhasználóhoz..."
sudo -u "$USERNAME" mkdir -p "$USER_HOME/.config/rofi"
cat > "$USER_HOME/.config/rofi/mission-impossible.sh" << 'EOF'
#!/bin/bash
...
chown "$USERNAME:$USERNAME" "$USER_HOME/.config/rofi/mission-impossible.sh"
chmod +x "$USER_HOME/.config/rofi/mission-impossible.sh"

# 5. Trigger hozzáadása a JELENLEGI FELHASZNÁLÓ .bashrc-jéhez
echo "Trigger hozzáadása a jelenlegi felhasználó .bashrc-jéhez..."
if ! sudo -u "$USERNAME" grep -q "mission-impossible.sh" "$USER_HOME/.bashrc"; then
...
# 6. AZONNALI VÉGREHAJTÁS a jelenlegi felhasználó számára - NEM FORRÁSOLJUK, HANEM KÖZVETLENÜL FUTTATJUK
echo "Azonnali végrehajtás a jelenlegi felhasználó számára..."
sudo -u "$USERNAME" bash << ENDOFSCRIPT
    echo "🕵️‍♂️ Executing Mission Impossible for current user (direct execution)..."
    if [ -f "/usr/local/bin/rofi-mission-impossible.sh" ]; then
        /usr/local/bin/rofi-mission-impossible.sh
    else
        echo "❌ Main mission script not found"
    fi
    echo "✅ Mission execution completed"
ENDOFSCRIPT

echo "✅ Mission Impossible telepítve:"
echo "   - Fő script: /usr/local/bin/rofi-mission-impossible.sh"
echo "   - Aktiváló: /etc/skel/.config/rofi/mission-impossible.sh (új felhasználóknak)"
echo "   - Aktiváló: $USER_HOME/.config/rofi/mission-impossible.sh (jelenlegi felhasználónak)"
echo "   - Trigger: .bashrc-be illesztve mindenkinek"
echo "   - Jelenlegi felhasználó számára AZONNAL lefuttatva!"
