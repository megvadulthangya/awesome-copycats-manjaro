...
REPO_PKGS=(
  ...
  arandr lxappearance lxqt-policykit xorg-xev terminus-font xss-lock wavemon network-manager-applet feh qt6ct
)
...
# --- AUR csomagok ---
AUR_PKGS=(
  lain-git awesome-git awesome-freedesktop-git tilix-git tamzen-font i3lock-fancy-git grayjay-bin betterlockscreen
)
...
# --- A FUNKCIÓ, AMI A BEÁLLÍTÁSOKAT ALKALMAZZA ---
# Bemenetként a cél mappát ($1) és a felhasználónevet ($2) várja
apply_gtk_settings() {
  local TARGET_HOME=$1
  local TARGET_USER=$2
...
gtk-theme-name="Matcha-dark-pueril"
...
EOF
  ...
  # FONTOS: A fájlok tulajdonosát beállítjuk, mert a sudo miatt root hozta őket létre
...
# A '~' itt automatikusan a /root mappára mutat, mert a script sudo-val fut
apply_gtk_settings ~ "root"

echo "Minden GTK beállítás sikeresen alkalmazva."
