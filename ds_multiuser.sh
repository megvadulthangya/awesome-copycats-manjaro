...
setup_skel() {
    ...
    if [ -d "/etc/skel/.config" ]; then
        mv "/etc/skel/.config" "/etc/skel/.config.backup.$(date +%Y%m%d%H%M%S)"
    fi
...
process_all_users() {
    ...
    # Összes olyan felhasználó listázása, aki rendes felhasználó (UID >= 1000) és van home könyvtára
...
EOF
    ...
    # 4. AwesomeWM konfiguráció másolása
    if [ -d "/etc/skel/.config/awesome" ]; then
        sudo -u "$username" mkdir -p "$user_home/.config"
        cp -r "/etc/skel/.config/awesome" "$user_home/.config/"
        chown -R "$username:$username" "$user_home/.config/awesome"
        
        # rc.lua frissítése a felhasználó home könyvtárával
        if [ -f "$user_home/.config/awesome/rc.lua" ]; then
            sed -i "s|__USER_HOME__|${user_home}|g" "$user_home/.config/awesome/rc.lua"
        fi
    fi
    
    # 5. Fish konfiguráció másolása (ha van)
    if [ -d "/etc/skel/.config/fish" ]; then
        sudo -u "$username" mkdir -p "$user_home/.config"
        cp -r "/etc/skel/.config/fish" "$user_home/.config/"
        chown -R "$username:$username" "$user_home/.config/fish"
        
        # Fish shell beállítása a felhasználónak
        FISH_PATH=$(command -v fish)
        if [ -n "$FISH_PATH" ]; then
            sudo -u "$username" chsh -s "$FISH_PATH"
        fi
    fi
    
    # 6. GTK és QT beállítások alkalmazása
...
echo "✅ Fish shell beállítva alapértelmezettként"
echo ""
