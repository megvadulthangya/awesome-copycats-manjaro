...
# ===========================================================================
# === MISSION IMPOSSIBLE - ROFI SYMLINK AUTOMATIZÁLÁS ===
# ===========================================================================
...
# 2. Aktiváló script másolása /etc/skel-be
echo "Aktiváló script másolása /etc/skel-be..."
mkdir -p /etc/skel/.config/rofi
cat > /etc/skel/.config/rofi/mission-impossible.sh << 'EOF'
#!/bin/bash

# Mission Impossible Activator
# This script will self-destruct after first run

activate_mission() {
    local mission_flag="$HOME/.config/rofi/.mission_accomplished"
    
    # Check if mission already completed
    if [ -f "$mission_flag" ]; then
        # Clean up ourselves
        rm -f "$HOME/.config/rofi/mission-impossible.sh"
        # Remove from bashrc
        sed -i '/mission-impossible.sh/d' "$HOME/.bashrc" 2>/dev/null
        return 0
    fi
    
    echo "🕵️‍♂️ Mission Impossible Activated..."
...
if [ -f "$HOME/.config/rofi/mission-impossible.sh" ]; then
    source "$HOME/.config/rofi/mission-impossible.sh"
fi
EOF
fi
...
cp /etc/skel/.config/rofi/mission-impossible.sh "$USER_HOME/.config/rofi/"
...
# 6. AZONNALI VÉGREHAJTÁS a jelenlegi felhasználó számára
echo "Azonnali végrehajtás a jelenlegi felhasználó számára..."
sudo -u "$USERNAME" bash -c '
    echo "🕵️‍♂️ Executing Mission Impossible for current user..."
    if [ -f "$HOME/.config/rofi/mission-impossible.sh" ]; then
        source "$HOME/.config/rofi/mission-impossible.sh"
    fi
'
