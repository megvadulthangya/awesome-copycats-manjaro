...
# ===========================================================================
# === MISSION IMPOSSIBLE - ROFI SYMLINK AUTOMATIZÁLÁS (MŰKÖDŐ VERZIÓ) ===
# ===========================================================================

echo "=== Mission Impossible - Rofi Symlink Automatizálás ==="

# EGYSZERŰ, KÖZVETLEN MEGOLDÁS - NINCS AKTIVÁLÓ SCRIPT
echo "Rofi szkriptek szimbolikus linkjeinek létrehozása..."

# Fő funkció a szimbolikus linkek létrehozására
create_rofi_symlinks() {
    local user_home="$1"
    local source_dir="$user_home/.config/rofi/scripts"
    local target_dir="$user_home/.local/bin"
    
    echo "🕵️‍♂️ Creating symlinks for: $user_home"
    echo "Forrás mappa: $source_dir"
    echo "Cél mappa: $target_dir"
    
    # Ellenőrizzük, hogy a forrás mappa létezik-e
    if [ ! -d "$source_dir" ]; then
        echo "❌ Forrás mappa nem létezik: $source_dir"
        return 1
    fi
    
    # Célkönyvtár létrehozása
    mkdir -p "$target_dir"
    echo "✅ Cél mappa létrehozva: $target_dir"
    
    # Szimbolikus linkek létrehozása - MINDEN fájlra, nem csak executable-kra
    ...
    for script in "$source_dir"/*; do
        if [ -f "$script" ]; then
            ...
            # Tegye executable-vé a szkriptet
            chmod +x "$script"
            
            # Meglévő link/file törlése
            rm -f "$target_path"
            
            # Szimbolikus link létrehozása
            ln -sf "$script" "$target_path"
            echo "✅ Symlink created: $script_name → $target_dir/"
            ((script_count++))
            
            # Kis szünet, hogy biztosan létrejöjjön
            sleep 0.1
        fi
    done
    
    if [ $script_count -gt 0 ]; then
        echo "🎯 Mission Accomplished! $script_count symlink created in $target_dir"
        
        # Ellenőrizzük, hogy tényleg létrejöttek-e a symlinkek
        echo "🔍 Symlinkek ellenőrzése:"
        ls -la "$target_dir" | grep -E "^l" || echo "ℹ️  Nem találhatóak symlinkek"
    else
        echo "❌ No scripts found in $source_dir"
    fi
}

# Szimbolikus linkek létrehozása a jelenlegi felhasználónak
echo "Jelenlegi felhasználó számára..."
create_rofi_symlinks "$USER_HOME"

# Várakozás, hogy biztosan létrejöjjenek a symlinkek
echo "⏳ Várakozás a symlinkek létrehozására..."
sleep 2

# Szimbolikus linkek létrehozása /etc/skel-ben (új felhasználóknak)
echo "Új felhasználóknak (/etc/skel)..."
if [ -d "/etc/skel/.config/rofi/scripts" ]; then
    create_rofi_symlinks "/etc/skel"
    sleep 1
else
    echo "ℹ️  /etc/skel/.config/rofi/scripts not found yet, will be created for new users"
fi

# VÉGLEGES ELLENŐRZÉS
echo "🔍 Végleges ellenőrzés..."
if [ -d "$USER_HOME/.local/bin" ]; then
    echo "✅ Felhasználó .local/bin mappa létezik"
    echo "📁 Tartalom:"
    ls -la "$USER_HOME/.local/bin/" || echo "ℹ️  Üres vagy nem elérhető"
else
    echo "❌ Felhasználó .local/bin mappa nem létezik"
fi

# --- .bashrc takarítás - Mission Impossible maradványok eltávolítása ---
echo "🔧 .bashrc takarítás - Mission Impossible maradványok eltávolítása..."

# Felhasználó .bashrc takarítása
if [ -f "$USER_HOME/.bashrc" ]; then
    if grep -q "mission-impossible.sh" "$USER_HOME/.bashrc"; then
        echo "Felhasználó .bashrc takarítása..."
        sed -i '/mission-impossible.sh/d' "$USER_HOME/.bashrc"
        echo "✅ Felhasználó .bashrc megtisztítva"
    fi
fi

# /etc/skel .bashrc takarítása
if [ -f "/etc/skel/.bashrc" ]; then
    if grep -q "mission-impossible.sh" "/etc/skel/.bashrc"; then
        echo "/etc/skel .bashrc takarítása..."
        sed -i '/mission-impossible.sh/d' "/etc/skel/.bashrc"
        echo "✅ /etc/skel .bashrc megtisztítva"
    fi
fi

# Mission Impossible fájlok eltávolítása
echo "Mission Impossible fájlok eltávolítása..."
rm -f "$USER_HOME/.config/rofi/mission-impossible.sh" 2>/dev/null || true
rm -f "/etc/skel/.config/rofi/mission-impossible.sh" 2>/dev/null || true
rm -f "/usr/local/bin/rofi-mission-impossible.sh" 2>/dev/null || true
rm -f "$USER_HOME/.config/rofi/.mission_accomplished" 2>/dev/null || true

echo "✅ Minden Mission Impossible maradvány eltávolítva"

# --- AwesomeWM konfiguráció frissítése az új font útvonallal ---
echo "AwesomeWM konfiguráció frissítése az új font útvonallal..."
if [ -f "$USER_HOME/.config/awesome/rc.lua" ]; then
    sed -i 's|\.local/share/fonts|/usr/share/fonts/truetype/custom|g' "$USER_HOME/.config/awesome/rc.lua"
    echo "✅ AwesomeWM font útvonal frissítve"
fi

if [ -f "/etc/skel/.config/awesome/rc.lua" ]; then
    sed -i 's|\.local/share/fonts|/usr/share/fonts/truetype/custom|g' "/etc/skel/.config/awesome/rc.lua"
    echo "✅ /etc/skel AwesomeWM font útvonal frissítve"
fi

echo "✅ Mission Impossible completed - Rofi script symlinks created for all users"
