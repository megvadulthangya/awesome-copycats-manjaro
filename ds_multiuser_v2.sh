...
# ===========================================================================
# === AUTOMATIKUS FELHASZNÁLÓKEZELÉS - MINDEN FELHASZNÁLÓ SZÁMÁRA ===
# ===========================================================================

# --- /etc/skel BEÁLLÍTÁSA (új felhasználók számára) ---
setup_skel() {
    echo "=== /etc/skel beállítása új felhasználók számára ==="

    # Alap könyvtárak létrehozása, ha nem léteznek - NE TÖRÖLJÜK A MEGLÉVŐEKET!
    mkdir -p /etc/skel/.config
    mkdir -p /etc/skel/.local/share/fonts

    # AwesomeWM konfiguráció másolása
    if [ -d "$USER_HOME/.config/awesome" ]; then
        echo "AwesomeWM konfiguráció másolása /etc/skel-be..."
        # Csak akkor másolunk, ha nem létezik, vagy biztonsági mentést készítünk
        if [ -d "/etc/skel/.config/awesome" ]; then
            mv "/etc/skel/.config/awesome" "/etc/skel/.config/awesome.backup.$(date +%Y%m%d%H%M%S)"
        fi
    ...
    # Fish konfiguráció másolása
    if [ -d "$USER_HOME/.config/fish" ]; then
        echo "Fish konfiguráció másolása /etc/skel-be..."
        if [ -d "/etc/skel/.config/fish" ]; then
            mv "/etc/skel/.config/fish" "/etc/skel/.config/fish.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp -r "$USER_HOME/.config/fish" /etc/skel/.config/
    fi

    # Rofi konfiguráció másolása - EZ HIÁNYZOTT!
    if [ -d "$USER_HOME/.config/rofi" ]; then
        echo "Rofi konfiguráció másolása /etc/skel-be..."
        if [ -d "/etc/skel/.config/rofi" ]; then
            mv "/etc/skel/.config/rofi" "/etc/skel/.config/rofi.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp -r "$USER_HOME/.config/rofi" /etc/skel/.config/
    else
        echo "Figyelem: $USER_HOME/.config/rofi nem található, rofi konfiguráció kihagyva"
    fi

    # Alap fájlok másolása - csak hozzáfűzzük, ha már van .xprofile
    if [ ! -f /etc/skel/.xprofile ]; then
        cat > /etc/skel/.xprofile << 'EOF'
export QT_QPA_PLATFORMTHEME="qt6ct"
EOF
    else
        # Ha már van, csak akkor írjuk felül, ha nem tartalmazza a sort
        if ! grep -q "QT_QPA_PLATFORMTHEME" /etc/skel/.xprofile; then
            echo 'export QT_QPA_PLATFORMTHEME="qt6ct"' >> /etc/skel/.xprofile
        fi
    fi

    # Font másolása
    if [ ! -f "/etc/skel/.local/share/fonts/Icons.bdf" ]; then
        curl -fL -o "/etc/skel/.local/share/fonts/Icons.bdf" \
            https://raw.githubusercontent.com/lcpz/dots/refs/heads/master/.fonts/Icons.bdf 2>/dev/null || true
    fi

    echo "✅ /etc/skel beállítva - új felhasználók automatikusan megkapják a beállításokat"
    echo "ℹ️  Meglévő konfigurációk megmaradtak, csak az AwesomeWM, Fish és Rofi beállítások frissültek"
}

# --- ÖSSZES LÉTEZŐ FELHASZNÁLÓ FELDOLGOZÁSA ---
process_all_users() {
    echo "=== Összes felhasználó feldolgozása ==="

    # Összes olyan felhasználó listázása, aki rendes felhasználó (UID >= 1000) és van home könyvtárra
    getent passwd | while IFS=: read -r username _ uid _ _ home shell; do
        # Csak rendes felhasználók (UID >= 1000), érvényes shell-lel és home könyvtárral
        if [ "$uid" -ge 1000 ] && [ "$uid" -lt 65534 ] &&
           [ "$shell" != "/bin/false" ] && [ "$shell" != "/usr/sbin/nologin" ] &&
           [ -d "$home" ]; then

            echo "Felhasználó feldolgozása: $username (UID: $uid, HOME: $home)"

            # Itt hívjuk meg a felhasználói beállításokat
            apply_user_settings "$username" "$home"
        fi
    done
}

# --- FELHASZNÁLÓI BEÁLLÍTÁSOK ALKALMAZÁSA ---
apply_user_settings() {
    local username="$1"
    local user_home="$2"

    echo "Beállítások alkalmazása a(z) $username felhasználóra..."

    # 1. Ikon font telepítése
    sudo -u "$username" mkdir -p "$user_home/.local/share/fonts"
    if [ ! -f "$user_home/.local/share/fonts/Icons.bdf" ]; then
        sudo -u "$username" curl -fL -o "$user_home/.local/share/fonts/Icons.bdf" \
            https://raw.githubusercontent.com/lcpz/dots/refs/heads/master/.fonts/Icons.bdf 2>/dev/null || true
    fi
    sudo -u "$username" fc-cache -fv "$user_home/.local/share/fonts"

    # 2. .xprofile létrehozása - csak hozzáfűzzük, nem írjuk felül
    if [ ! -f "$user_home/.xprofile" ]; then
        sudo -u "$username" bash -c "cat > \"$user_home/.xprofile\"" << 'EOF'
export QT_QPA_PLATFORMTHEME="qt6ct"
EOF
    else
        # Ha már van, csak akkor adjuk hozzá, ha nem tartalmazza
        if ! sudo -u "$username" grep -q "QT_QPA_PLATFORMTHEME" "$user_home/.xprofile"; then
            sudo -u "$username" bash -c "echo 'export QT_QPA_PLATFORMTHEME=\"qt6ct\"' >> \"$user_home/.xprofile\""
        fi
    fi

    # 3. Flameshot beállítások - csak akkor, ha nincs
    sudo -u "$username" mkdir -p "$user_home/.config/flameshot"
    if [ ! -f "$user_home/.config/flameshot/flameshot.ini" ]; then
        sudo -u "$username" bash -c "cat > \"$user_home/.config/flameshot/flameshot.ini\"" << 'EOF'
[General]
contrastOpacity=188
drawColor=#ff0000
uiColor=#f5eef8
EOF
    ...
    # 4. AwesomeWM konfiguráció másolása - csak akkor, ha nincs
    if [ -d "/etc/skel/.config/awesome" ] && [ ! -d "$user_home/.config/awesome" ]; then
        sudo -u "$username" mkdir -p "$user_home/.config"
        sudo -u "$username" cp -r "/etc/skel/.config/awesome" "$user_home/.config/"
        sudo -u "$username" chown -R "$username:$username" "$user_home/.config/awesome"

        # rc.lua frissítése a felhasználó home könyvtárával
        if [ -f "$user_home/.config/awesome/rc.lua" ]; then
            sudo -u "$username" sed -i "s|__USER_HOME__|${user_home}|g" "$user_home/.config/awesome/rc.lua"
        fi
        echo "  ✅ AwesomeWM konfiguráció másolva"
    elif [ -d "$user_home/.config/awesome" ]; then
        echo "  ℹ️  AwesomeWM konfiguráció már létezik, nem frissítjük"
    fi

    # 5. Fish konfiguráció másolása - csak akkor, ha nincs
    if [ -d "/etc/skel/.config/fish" ] && [ ! -d "$user_home/.config/fish" ]; then
        sudo -u "$username" mkdir -p "$user_home/.config"
        sudo -u "$username" cp -r "/etc/skel/.config/fish" "$user_home/.config/"
        sudo -u "$username" chown -R "$username:$username" "$user_home/.config/fish"
        echo "  ✅ Fish konfiguráció másolva"
    elif [ -d "$user_home/.config/fish" ]; then
        echo "  ℹ️  Fish konfiguráció már létezik, nem frissítjük"
    fi

    # 6. Rofi konfiguráció másolása - EZ HIÁNYZOTT! - csak akkor, ha nincs
    if [ -d "/etc/skel/.config/rofi" ] && [ ! -d "$user_home/.config/rofi" ]; then
        sudo -u "$username" mkdir -p "$user_home/.config"
        sudo -u "$username" cp -r "/etc/skel/.config/rofi" "$user_home/.config/"
        sudo -u "$username" chown -R "$username:$username" "$user_home/.config/rofi"
        echo "  ✅ Rofi konfiguráció másolva"
    elif [ -d "$user_home/.config/rofi" ]; then
        echo "  ℹ️  Rofi konfiguráció már létezik, nem frissítjük"
    fi

    # 7. GTK és QT beállítások alkalmazása
    echo "  GTK és QT beállítások alkalmazása..."
    apply_gtk_settings "$user_home" "$username"
    apply_qt_settings "$user_home" "$username"

    echo "✅ $username felhasználó beállításai kész"
}

# --- FŐ PROGRAM FOLYTATÁSA ---

# 1. /etc/skel beállítása (új felhasználóknak)
setup_skel

# 2. Összes létező felhasználó feldolgozása
process_all_users
...
echo "✅ Összes létező felhasználó megkapta a beállításokat"
echo "✅ Új felhasználók automatikusan megkapják a kinézetet"
echo "✅ Nordic téma beállítva mindenhol (GTK, QT, Kvantum, AwesomeWM, LightDM)"
echo "✅ Fish konfiguráció másolva (de nem alapértelmezett shell)"
echo ""
echo "ℹ️  Fontos:"
echo "   - A meglévő felhasználók XFCE beállításai megmaradtak"
echo "   - Csak az AwesomeWM, Fish és Rofi konfigurációk kerültek frissítésre"
echo "   - Új felhasználók teljes körű konfigurációt kapnak"
echo ""
