...
create_rofi_symlinks() {
    ...
    # Szimbolikus linkek létrehozása - find és while loop a space-ek kezelésére
    local script_count=0
    find "$source_dir" -type f -print0 | while IFS= read -r -d '' script; do
        if [ -f "$script" ]; then
...
# NE töröljük a mission impossible fájlokat
echo "ℹ️  Mission Impossible fájlok megtartva..."
